<template>
	<view class="imt-audio" :class="[`${theme}`]" v-if="audiolist.length > 0">
		...
		<template v-if="theme == 'theme1'"  >
			<view class="top" >
				<view  style="margin-top: 0px;" class="audio-control-wrapper"><image :src="renderData('coverImgUrl')" mode="aspectFill" class="cover" :class="{ on: !renderData('paused') }"></image></view>
			...
			<view class="audio-button-box">
				...
				<div class="playbox">
					<image :src="require('./static/loading2.png')" v-if="false" class="play loading"></image>
					<template v-else>
						<!-- 播放 -->
