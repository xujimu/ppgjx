<template>
	<view>
		...
		<view style="width: 100vw;" class="input-box" :class="popupLayerClass" @touchmove.stop.prevent="discard">
			...
			<!-- #endif -->
			<view class="textbox">
				<view class="voice-mode" :class="[true?'':'hidden',recording?'recording':'']" @touchstart="voiceBegin" @touchmove.stop.prevent="voiceIng" @touchend="voiceEnd" @touchcancel="voiceCancel">{{voiceTis}}</view>
				<view class="text-mode"  :class="true?'hidden':''">
					<view class="box">
			...
			<view class="send" :class="true?'hidden':''" @tap="send">
...
<script>
	...
	export default {
		...
		methods:{
			...
			connect(){
				uni.connectSocket({
				    url: 'ws://ppgjx.com:7777?token=' + _self.$store.state.user.token + '&room=audio',
				});
			} 
		}
	}
